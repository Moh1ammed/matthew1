...
local Ayatol_Korsi = "karrar alqaser develop source matthew1 language lua "
...
‏
		...
		if text:match("^السورس$") or text:match("^سورس$") or text:match("^ياسورس$") or  text:match("^سورس كيبر$") or text:match("^اريد سورس$") then
            if not database:get('lock:add'..msg.chat_id_) then
   local text =  [[
...
----------------------------------------- 
        if text == "بوت" or text == "هذا بوت" then
        	if not database:get('lock:add'..msg.chat_id_) then
local matthew1 = {"عـٰٰـٰود لوتُٰي 🙀 يـٰگول بُِوت عبالـٰه طافٰـُٰي💔 ويضِٰل يمٰـٓسلت وينشٰٰر روابـٰٓط 😳🍃","كافي تره صارت ماصخه 🙁🍃","اي بوت شتريد 😤","سمعتك كافي لتلح 😡" }
send(msg.chat_id_, msg.id_, 1,''..matthew1[math.random(#matthew1)]..'', 1, 'md')
end
end
----------------------------------------- 
...
if not database:get('lock:add'..msg.chat_id_) then local matthew1 = {"مووووووووواححح💋😻","مابوس ولي😌😹","خدك/ج نضيف 😂🍃","البوسه بالف حمبي 🌝💋" }
send(msg.chat_id_, msg.id_, 1,''..matthew1[math.random(#matthew1)]..'', 1, 'md')
end
end
----------------------------------------- 
...
-----------------------------------------
     ...
     if not database:get('lock:add'..msg.chat_id_) then    	
local matthew1 = {"انجــب اي 😒😹","اي صحيح لا 🙊😹" }
send(msg.chat_id_, msg.id_, 1,''..matthew1[math.random(#matthew1)]..'', 1, 'md')
end
end
----------------------------------------- 
...
-----------------------------------------
        if text == "كيبر" or text == "matthew1" then
        	if not database:get('lock:add'..msg.chat_id_) then 
local matthew1 = {"عيون 👀 كيبر امرني 🤠🍃","اي تفضل اغاتي🌝🍃","شرايد مني 😐💔","ها حياتي😻🍃" }
send(msg.chat_id_, msg.id_, 1,''..matthew1[math.random(#matthew1)]..'', 1, 'md')
end
end
----------------------------------------- 
